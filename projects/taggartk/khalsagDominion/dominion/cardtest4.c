 ...
 * Class: CS 362
 * Date: May 5, 2019
 * Description: Fourth card test for Assignment 3, 2b
 *   This will be testing functionality of the Village card effect.
...
int main() {
  ...
  printf("\n---------- Card Test: Village ----------\n\n");
  printf("Test 1: Play Village from a fresh game state.\n");
  memcpy(&test_g, &g, sizeof(struct gameState));

  cardEffect(village, choice1, choice2, choice3, &test_g, handPos, &bonus);
  ...
  printf("Current deck count: %d; Expected deck count: %d\n", test_g.deckCount[current_player],
      g.deckCount[current_player] - 1);
  myAssert(test_g.deckCount[current_player] == g.deckCount[current_player] - 1);
  ...
  printf("Test 2: Play Village three times from a fresh game state.\n");
  memcpy(&test_g, &g, sizeof(struct gameState));
  ...
  printf("Current actions: %d; Expected actions: %d\n", test_g.numActions, g.numActions + 6);
  myAssert(test_g.numActions == g.numActions + 6);
  printf("Current coins: %d; Expected coins: %d\n", test_g.coins, g.coins);
  myAssert(test_g.coins == g.coins);
  printf("Current buys: %d; Expected buys: %d\n", test_g.numBuys, g.numBuys);
  myAssert(test_g.numBuys == g.numBuys);
