 ...
 * Class: CS 362
 * Date: May 5, 2019
 * Description: Fourth unit test for Assignment 3, 2a
 *   This will be testing functionality of the refactored Treasure Map card effect.
...
int main() {
  ...
  printf("\n---------- Unit Test: Treasure Map ----------\n\n");
  printf("Test 1: Play Treasure Map when there is no other Treasure Map in hand.\n");
  memcpy(&test_g, &g, sizeof(struct gameState));

  cardEffect(treasure_map, choice1, choice2, choice3, &test_g, handPos, &bonus);
  ...
  printf("Gold cards in supply: %d; Expected # in supply: %d\n", test_g.supplyCount[gold],
      g.supplyCount[gold]);
  myAssert(test_g.supplyCount[gold] == g.supplyCount[gold]);
  ...
  printf("\nTest 2: Play Treasure Map when there is another Treasure Map in hand.\n");
  memcpy(&test_g, &g, sizeof(struct gameState));

  // Seed hand with second Treasure Map
  test_g.hand[current_player][test_g.handCount[current_player] - 1] = treasure_map;
  cardEffect(treasure_map, choice1, choice2, choice3, &test_g, handPos, &bonus);

  printf("Checking game state of current player ---\n");
  printf("Current hand count: %d; Expected hand count: %d\n", test_g.handCount[current_player],
      g.handCount[current_player] - 2);
  myAssert(test_g.handCount[current_player] == g.handCount[current_player] - 2);
  printf("Current deck count: %d; Expected deck count: %d\n", test_g.deckCount[current_player],
      g.deckCount[current_player] + 4);
  myAssert(test_g.deckCount[current_player] == g.deckCount[current_player] + 4);
  printf("Current discard count: %d; Expected discard count: %d\n", test_g.discardCount[current_player],
      g.discardCount[current_player]);
  myAssert(test_g.discardCount[current_player] == g.discardCount[current_player]);
  printf("Current actions: %d; Expected actions: %d\n", test_g.numActions, g.numActions);
  ...
  myAssert(test_g.supplyCount[gold] == g.supplyCount[gold] - 4);
  printf("Total cards in supply: %d; Expected cards in supply: %d\n", getSupplyCount(&test_g),
      getSupplyCount(&g) - 4);
  myAssert(getSupplyCount(&test_g) == getSupplyCount(&g) - 4);
