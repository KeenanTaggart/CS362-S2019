 ...
 * Class: CS 362
 * Date: May 5, 2019
 * Description: Second card test for Assignment 3, 2b
 *   This will be testing functionality of the Adventurer card effect.
...
int main() {
  ...
  printf("Test 1: Play Adventurer with two treasures on top of deck.\n");
  memcpy(&test_g, &g, sizeof(struct gameState));

  test_g.deck[current_player][test_g.deckCount[current_player] - 1] = copper;
  test_g.deck[current_player][test_g.deckCount[current_player] - 2] = copper;
  ...
  printf("\nTest 2: Play Adventurer with treasure, action, treasure on top of deck.\n");
  memcpy(&test_g, &g, sizeof(struct gameState));

  test_g.deck[current_player][test_g.deckCount[current_player] - 1] = copper;
  test_g.deck[current_player][test_g.deckCount[current_player] - 2] = adventurer;
  test_g.deck[current_player][test_g.deckCount[current_player] - 3] = copper;
  cardEffect(adventurer, choice1, choice2, choice3, &test_g, handPos, &bonus);
  ...
  printf("Current discard count: %d; Expected discard count: %d\n", test_g.discardCount[current_player],
      g.discardCount[current_player] + 2);
  ...
  printf("\nTest 3: Play Adventurer with action, action, treasure, treasure on top of deck.\n");
  memcpy(&test_g, &g, sizeof(struct gameState));

  test_g.deck[current_player][test_g.deckCount[current_player] - 1] = adventurer;
  test_g.deck[current_player][test_g.deckCount[current_player] - 2] = adventurer;
  test_g.deck[current_player][test_g.deckCount[current_player] - 3] = copper;
  test_g.deck[current_player][test_g.deckCount[current_player] - 4] = copper;
  cardEffect(adventurer, choice1, choice2, choice3, &test_g, handPos, &bonus);
